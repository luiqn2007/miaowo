...
object HttpUtil {
    ...
    @JvmOverloads fun post(request: Request, error: (call: Call, e: IOException) -> Unit = { _, e -> activity?.handleError(e) }, onUI: Boolean = true, callback: (call: Call, response: Response) -> Unit): Call {
        val call = client.newCall(request)
        lInfo("url: ${request.url()}")
        call.enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                if (onUI) {
                    activity?.uiThread { error(call, e) }
                } else {
                    error(call, e)
                }
            }
            override fun onResponse(call: Call, response: Response) {
                if (onUI) {
                    activity?.uiThread { callback(call, response) }
                } else {
